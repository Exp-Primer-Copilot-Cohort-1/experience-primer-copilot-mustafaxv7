...
def CeaserCipher(sentence , publicKey = 0):# code
    sentence = sentence.split(" ")
    Ciphered_sentence = " "
    for i in sentence:
        Ciphered_sentence+= " "
        for letter in i.lower():
            Ciphered_sentence += chr(ord(letter) + publicKey)
...
while True:
    ...
    if option == 1:
        ...
        publicKey = int(input("\n\nEnter The PublicKey of Protection >>: "))
        # privateKey = int(input("\n\nEnter The PrivateKey of Protection >>: "))
        print(f"\n\nThe Protected Message is >>> \t\"\"{CeaserCipher(sentece_to_protect,publicKey)} \"\"\n\n\n")
    elif option == 2:
